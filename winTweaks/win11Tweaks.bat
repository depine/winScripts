...
rem echo Ativando Windows Photo Viewer...
...
echo Desativando arquivos do Office.com em Acesso Rapido (win11)...
...
echo Ocultando icone Reunir Agora (win10)...
...
echo Posicionando Menu Iniciar a esquerda (win11)...
REG ADD "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v TaskbarAl /t REG_DWORD /d 0 /f >nul 2>&1

REM echo Ocultando Noticias e Clima (win10)...
REM REG ADD "HKLM\SOFTWARE\Microsoft\PolicyManager\default\NewsAndInterests" /v AllowNewsAndInterests /t REG_DWORD /d 0 /f >nul 2>&1
echo.
...
echo Desativando Apps mais usados... 
REM No Windows 11 a otimizacao abaixo desativa a opcao
REM REG ADD "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v Start_TrackProgs /t REG_DWORD /d 0 /f >nul 2>&1
echo Desativando apps adicionados recentemente no menu iniciar...
REG ADD "HKCU\SOFTWARE\Policies\Microsoft\Windows\Explorer" /v HideRecentlyAddedApps /t REG_DWORD /d 1 /f >nul 2>&1

echo Desativando recomendações para dicas, atalhos, novos aplicativos e muito mais
...
echo Desativando notificações relacionadas a conta...
